  ...
  const handleUploadComplete = useCallback(
    async (file: File, localUrl: string) => {
      ...
      try {
        ...
        toast({
          title: "Upload Failed",
          description: "Could not connect to backend. Using local preview.",
          variant: "destructive",
        });
  ...
  const handleSegment = useCallback(async () => {
    ...
    try {
      const result = await segmentFrame({
        ...
        polygon: annotations.polygons.flatMap((poly) =>
          poly.map((p) => [p.x, p.y]),
        ),
      });
      ...
      toast({
        ...
        description: "Could not connect to segmentation backend.",
        variant: "destructive",
      });
      setStatus("ready");
    }
  ...
  const handleTrack = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Masks propagated across all frames.",
      });
    } catch (err) {
      toast({
        title: "Tracking Failed",
        description: "Could not connect to tracking backend.",
        variant: "destructive",
      });
      setStatus("segmented");
    }
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-border bg-card/50 backdrop-blur-sm">
        ...
        {videoId && (
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span>
              Video ID:{" "}
              <span className="font-mono text-primary">{videoId}</span>
            </span>
        ...
        {(status === "idle" || isUploading) && (
          <VideoUploader
            ...
            status={
              status === "idle"
                ? "idle"
                : (status as "uploading" | "extracting")
            }
          />
        )}
